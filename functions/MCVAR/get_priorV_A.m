function sigmab = get_priorV_A(omega,gamma,phi,index_dom,index_di,N,G,L)
...
for ii=1:N
    ...
    for jj=1:G
        ...
        ind=jj:G:G*L;
        inda(ind)=[];
        sigmab(indexc(ind), indx_v(jj))= phi*local_V(indexc(ind), indx_v(jj));
        sigmab(indexc(inda), indx_v(jj))=phi*local_V(indexc(inda), indx_v(jj));
    end
end
