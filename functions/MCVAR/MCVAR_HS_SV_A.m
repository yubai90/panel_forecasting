function Y_pred =MCSVAR_HS_SV_A(data,reps,burnin,N,G,L,h)
...
% Prior is Horseshoe and all parameters are combined together.
...
for irep=1:reps
   ...
   % PAI=triang(Y,X,NG,K_eq,T,invA_,sqrt_ht,iV,iVb_prior); 
    ...
    for jt=1:N
        for ie=1:nd*G
            Gamma_di(ie,jt)=gamrnd(1,1/(lambda(1)+Omega_di(ie,jt)));
        end
     end
    for ie=1:m
        Gamma_a(ie)=gamrnd(1,1/(lambda(2)+Omega_a(ie)));
    end
    
    % Step 5 and 6: Draw lambda and c from hiearchical representation
    lambda(1)=gamrnd(1+(K_eq*NG-1)/2,1/(c(1)+sum([Gamma_int;Gamma_olag;Gamma_cvlag;Gamma_di(:)])));
    c(1)=gamrnd(1,1/(lambda(1)+1));
   
    
    lambda(2)=gamrnd(1+(m-1)/2,1/(c(2)+sum(Gamma_a)));
