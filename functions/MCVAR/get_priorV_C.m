function sigmab = get_priorV_C(omega,gamma,phi,index_dom,index_di,N,G,L)
...
% 2c/(ab)(we have N+1, 1 for CSH, N for DI).

local_V=omega./gamma;
[K,NG]=size(local_V);
sigmab=zeros(K,NG);
sigmab(1,:)=phi(1)*local_V(1,:);
...
for ii=1:N
    ...
    for jj=1:G
        ...
        ind=jj:G:G*L;
        inda(ind)=[];
        sigmab(indexc(ind), indx_v(jj))= phi(1)*local_V(indexc(ind), indx_v(jj));
        sigmab(indexc(inda), indx_v(jj))=phi(1)*local_V(indexc(inda), indx_v(jj));
    end
end
...
for ii=1:NG
   ...
   % sigmab(ind_csh,ii)=phi(1)*csh_temp;
    ...
    sigmab(ind_di,ii)=phi(1+inc)*dip;
end
