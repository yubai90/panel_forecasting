function sigmab = get_priorV(omega,gamma,phi,index_dom,index_di,N,G,L)
...
for ii=1:N
    ...
    for jj=1:G
        ...
        ind=jj:G:G*L;
        inda(ind)=[];
        sigmab(indexc(ind), indx_v(jj))= phi(2)*local_V(indexc(ind), indx_v(jj));
        sigmab(indexc(inda), indx_v(jj))=phi(3)*local_V(indexc(inda), indx_v(jj));
    end
end
...
for ii=1:NG
   ...
   % sigmab(ind_csh,ii)=phi(1)*csh_temp;
    ...
    sigmab(ind_di,ii)=phi(3+inc)*dip;
end
